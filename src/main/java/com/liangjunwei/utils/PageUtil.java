...
public final class PageUtil {
	 ...
	 * @param pageNumSize 每页显示的数据条数
	 * @param list     显示的list集合
	 * @param listCount 一共有多少条数据
	 * @param pageNum     当前页码
	 */
	public static void pageNum(HttpServletRequest request, String url, Integer pageSize, List<?> list, Integer listCount, Integer pageNum) {
		// 通过符合要求的总条数和页面显示数来计算总页数
		int pageNumCount = listCount/pageSize + (listCount%pageSize == 0 ? 0 : 1);
		//System.out.println("pageNumCount=="+pageNumCount);
		...
		if(pageNum != pageNumCount) {
			last = "<a href='"+request.getContextPath()+url+flag+"pageNum="+pageNumCount+"'>末页</a>";
		} else {
			last = "末页";
		}
